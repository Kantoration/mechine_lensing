...
from src.validation.physics_validator import PhysicsValidator
...
class PhysicsEnsembleEvaluator:
    ...
    def evaluate(
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(data_loader):
                ...
                all_member_predictions.append(
                    outputs["member_predictions"].cpu().numpy()
                )
        ...
        if all_physics_analyses and "attention_maps" in all_physics_analyses[0]:
            ...
            attention_metrics = self._analyze_attention_maps(
                all_physics_analyses, labels
            )
    ...
    def _analyze_physics_consistency(
        self, physics_analyses: List[Dict[str, Any]]
    ) -> Dict[str, float]:
        ...
        for analysis in physics_analyses:
            ...
            physics_correlations.append(
                physics_consistency["physics_traditional_correlation"]
            )
    ...
    def _analyze_uncertainties(
        self, predictions: np.ndarray, labels: np.ndarray, uncertainties: np.ndarray
    ) -> Dict[str, float]:
        ...
        uncertainty_confidence_corr = np.corrcoef(
            mean_uncertainty, prediction_confidence
        )[0, 1]
        ...
        return {
            ...
            "uncertainty_correct_mean": np.mean(uncertainty_correct)
            if len(uncertainty_correct) > 0
            else 0.0,
            "uncertainty_incorrect_mean": np.mean(uncertainty_incorrect)
            if len(uncertainty_incorrect) > 0
            else 0.0,
            "uncertainty_discrimination": np.mean(uncertainty_incorrect)
            - np.mean(uncertainty_correct)
    ...
    def _analyze_ensemble_members(
        ...
        for i, name in enumerate(member_names):
            ...
            metrics = compute_classification_metrics(
                y_true=labels, y_pred=preds, y_pred_binary=(preds > 0.5).astype(int)
            )
    ...
    def _analyze_attention_maps(
        self, physics_analyses: List[Dict[str, Any]], labels: np.ndarray
    ) -> Dict[str, float]:
                    ...
                    if "enhanced_light_transformer" in model_name:
                        for map_name, attention_map in maps.items():
                            if attention_map.size > 0:
                                ...
                                entropy = -np.sum(
                                    attention_map * np.log(attention_map + 1e-8)
                                )
        ...
        return {
            "mean_attention_entropy": np.mean(attention_qualities)
            if attention_qualities
            else 0.0,
            "attention_physics_consistency": np.mean(physics_consistencies)
            if physics_consistencies
            else 0.0,
        }
    ...
    def _create_summary(
        ...
        return {
            ...
            "uncertainty_discrimination": uncertainty_metrics[
    ...
    def _save_detailed_predictions(
        ...
        np.savez(self.save_dir / "detailed_predictions.npz", **detailed_results)

        logger.info(
            f"Saved detailed predictions to {self.save_dir / 'detailed_predictions.npz'}"
        )
    ...
    def _create_visualizations(
        ...
        self._plot_uncertainty_analysis(
            results["uncertainty_metrics"], physics_analyses, labels
        )

    def _plot_member_performance(
        ...
        plt.tight_layout()
        plt.savefig(
            self.save_dir / "member_performance.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def _plot_physics_consistency(self, physics_metrics: Dict[str, float]) -> None:
        ...
        ax1.bar(
            ["Physics Consistency Score"],
            [physics_metrics["physics_consistency_score"]],
        )
        ...
        ax2.bar(
            ["Physics-Traditional Correlation"],
            [physics_metrics["mean_physics_correlation"]],
        )
        ...
        plt.tight_layout()
        plt.savefig(
            self.save_dir / "physics_consistency.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def _plot_uncertainty_analysis(
        ...
        ax2.boxplot(
            [nonlens_uncertainties, lens_uncertainties], labels=["Non-Lens", "Lens"]
        )
        ...
        plt.tight_layout()
        plt.savefig(
            self.save_dir / "uncertainty_analysis.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
...
def main():
    ...
    test_dataset = LensDataset(
        data_root=args.data_root, split="test", transform_config={"resize": 112}
    )
    ...
    evaluator = PhysicsEnsembleEvaluator(
        model=ensemble, device=device, save_dir=Path(args.save_dir)
    )

    # Run evaluation
    results = evaluator.evaluate(
        data_loader=test_loader, visualize=args.visualize, save_predictions=True
    )
