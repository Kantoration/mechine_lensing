...
class GeneralConfig:
    ...
    balance: float = 0.5
    backend: str = (
        "synthetic"  # Backend to use: "synthetic", "deeplenstronomy", or "auto"
    )
    ...
    def __post_init__(self) -> None:
        ...
        if self.image_size < 8:
            raise ValueError(
                f"image_size too small for meaningful features, got {self.image_size}"
            )
        ...
        if self.backend not in ["synthetic", "deeplenstronomy", "auto"]:
            raise ValueError(
                f"backend must be 'synthetic', 'deeplenstronomy', or 'auto', got '{self.backend}'"
            )
...
def atomic_write(target_path: Path, mode: str = "w", **kwargs) -> Iterator[Any]:
    ...
    temp_fd, temp_path = tempfile.mkstemp(
        dir=target_path.parent, prefix=f".tmp_{target_path.name}_", suffix=".tmp"
    )
...
class SyntheticImageGenerator:
    ...
    def create_lens_arc_image(
        self, image_id: str, split: str
        ...
        galaxy_sigma = self.rng.uniform(
            arc_config.galaxy_sigma_min, arc_config.galaxy_sigma_max
        )
        galaxy_ellipticity = self.rng.uniform(
            arc_config.galaxy_ellipticity_min, arc_config.galaxy_ellipticity_max
        )
        galaxy_angle = self.rng.uniform(0, np.pi)
        galaxy_brightness = self.rng.uniform(
            arc_config.galaxy_brightness_min, arc_config.galaxy_brightness_max
        )
        ...
        for _ in range(n_arcs):
            ...
            arc_length = self.rng.uniform(
                np.pi / 4, np.pi / 2
            )  # Shorter arcs (more realistic)
            ...
            brightness = self.rng.uniform(
                arc_config.brightness_min, arc_config.brightness_max
            )
            ...
            for i in range(n_segments):
                ...
                arc_gaussian = np.exp(
                    -0.5
                    * (((x_arc - arc_x) / width) ** 2 + ((y_arc - arc_y) / width) ** 2)
                )
    ...
    def create_galaxy_blob_image(
        ...
        n_components = self.rng.integers(
            blob_config.n_components_min, blob_config.n_components_max + 1
        )
        ...
        for comp_i in range(n_components):
            ...
            ellipticity = self.rng.uniform(
                blob_config.ellipticity_min, blob_config.ellipticity_max
            )
            ...
            if comp_i == 0:
                brightness = self.rng.uniform(
                    blob_config.brightness_min, blob_config.brightness_max
                )
            else:
    ...
    def _generate_split(
        self, n_images: int, split: str, output_dir: Path, balance: float
    ) -> None:
        ...
        logger.info(
            f"Generating {split} split: {n_lens} lens + {n_nonlens} non-lens images"
        )
        ...
        for i in range(n_lens):
            ...
            if output.create_split_subdirs and output.create_class_subdirs:
                img_path = (
                    ...
                    / split
                    / "lens"
                    / f"{image_id}.{output.image_format.lower()}"
                )
            else:
            ...
            atomic_save_image(
                img_pil,
                img_path,
                format=output.image_format,
                quality=output.image_quality,
            )

            # Track metadata
            metadata.filename = (
                str(img_path.relative_to(output_dir))
                if output.relative_paths
                else str(img_path)
            )
        ...
        for i in range(n_nonlens):
            ...
            if (i + 1) % 100 == 0:
                logger.debug(
                    f"Generated {i + 1}/{n_nonlens} non-lens images for {split}"
                )
...
def create_csv_files(
    output_dir: Path, config: DatasetConfig, metadata_tracker: MetadataTracker
) -> None:
    ...
    train_images = [
        p for p in all_images if "/train/" in str(p) or "\\train\\" in str(p)
    ]
    ...
    def create_csv_data(images: List[Path]) -> List[Dict[str, Any]]:
        ...
        for img_path in images:
            relative_path = (
                ...
                if config.output.relative_paths
                else img_path
            )
            record = {
...
def validate_dataset(output_dir: Path, config: DatasetConfig) -> None:
    ...
    sample_images = list(train_df["filepath"].head(5)) + list(
        test_df["filepath"].head(2)
    )
        ...
        try:
            with Image.open(img_path) as img:
                if config.validation.check_image_stats:
                    ...
                    if not (
                        config.validation.min_brightness
                        <= brightness
                        <= config.validation.max_brightness
                    ):
                        logger.warning(
                            f"Image {img_path} brightness {brightness:.3f} outside expected range"
                        )
...
Examples:
  ...
  python scripts/generate_dataset.py --validate --backend synthetic
    ...
    parser.add_argument("--out", type=Path, default="data", help="Output directory")
    parser.add_argument(
        "--backend",
        choices=["synthetic"],
        default="synthetic",
    ...
    try:
        ...
        logger.info(
            ...
            + (
                ", metadata.csv, config_snapshot.json"
