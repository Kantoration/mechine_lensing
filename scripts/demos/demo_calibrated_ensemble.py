...
def create_mock_ensemble() -> UncertaintyWeightedEnsemble:
    ...
    # Create ensemble
    ensemble = UncertaintyWeightedEnsemble(members=members, member_names=member_names)
...
def demonstrate_ensemble_prediction(
    ensemble: UncertaintyWeightedEnsemble, inputs: Dict[str, torch.Tensor]
) -> tuple:
...
def create_calibration_plots(
    logits: torch.Tensor, labels: torch.Tensor, scaler: TemperatureScaler
):
    ...
    reliability_diagram(
        ...
        save_path=Path("results/reliability_before.png"),
    )
    ...
    reliability_diagram(
        ...
        save_path=Path("results/reliability_after.png"),
    )

    print(
        "✅ Saved calibration plots to results/reliability_before.png and results/reliability_after.png"
    )


def demonstrate_uncertainty_analysis(
    pred_mean: torch.Tensor, pred_var: torch.Tensor, labels: torch.Tensor
):
    ...
    if correct_mask.sum() > 0 and incorrect_mask.sum() > 0:
        ...
        print(
            f"   Ratio (incorrect/correct): {uncertainty_incorrect / uncertainty_correct:.2f}x"
        )
