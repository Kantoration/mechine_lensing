...
def demo_mixed_precision_training():
    ...
    memory_reduction = (
        (metrics_fp32.gpu_memory_gb - metrics_amp.gpu_memory_gb)
        / metrics_fp32.gpu_memory_gb
        * 100
    )

    logger.info("📊 AMP Results:")
...
def main():
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Run quick demo (basic features only)"
    )
    parser.add_argument(
        "--full-demo", action="store_true", help="Run full demo (all features)"
    )
    parser.add_argument("--amp", action="store_true", help="Include AMP demonstrations")
