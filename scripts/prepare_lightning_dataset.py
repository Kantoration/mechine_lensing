...
def main():
    ...
    parser.add_argument(
        "--data-root", type=str, required=True, help="Root directory of local dataset"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument(
        "--image-size", type=int, default=224, help="Image size for compression"
    )
    ...
    parser.add_argument(
        "--validate", action="store_true", help="Validate dataset after creation"
    )
...
def validate_webdataset(shard_dir: Path) -> None:
    ...
    logger.info(
        f"Validation passed: {total_samples} samples in {len(shard_files)} shards"
    )
