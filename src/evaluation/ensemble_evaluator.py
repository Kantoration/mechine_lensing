...
def load_ensemble_models(
    cnn_weights: Path, vit_weights: Path, device: torch.device
...
def create_ensemble_dataloaders(
    data_root: str, batch_size: int = 64, num_workers: int = 2
...
def get_model_predictions(
    model: nn.Module, data_loader: DataLoader, device: torch.device, model_name: str
    ...
    with torch.no_grad():
        for batch_idx, (images, labels) in enumerate(data_loader):
            ...
            if batch_idx % 10 == 0:
                logger.debug(
                    f"{model_name} processed batch {batch_idx}/{len(data_loader)}"
                )
...
def print_ensemble_results(
    ...
    print("\nIndividual Model Performance:")
    print(
        f"  CNN (ResNet-18):  Accuracy={cnn_metrics['accuracy']:.4f}, AUC={cnn_metrics.get('roc_auc', 'N/A'):.4f}"
    )
    print(
        f"  ViT (ViT-B/16):   Accuracy={vit_metrics['accuracy']:.4f}, AUC={vit_metrics.get('roc_auc', 'N/A'):.4f}"
    )
    ...
    report = classification_report(
        y_true, ensemble_preds, target_names=class_names, digits=4
    )
    print(report)
...
def main():
    ...
    # Required arguments
    parser.add_argument(
        "--cnn-weights", type=str, required=True, help="Path to CNN model weights"
    )
    parser.add_argument(
        "--vit-weights", type=str, required=True, help="Path to ViT model weights"
    )
    ...
    if abs(args.cnn_weight + args.vit_weight - 1.0) > 1e-6:
        logger.warning(
            f"Ensemble weights don't sum to 1.0: {args.cnn_weight + args.vit_weight}"
        )
    ...
    try:
        ...
        y_true_cnn, cnn_probs = get_model_predictions(
            cnn_model, cnn_loader, device, "CNN"
        )
        y_true_vit, vit_probs = get_model_predictions(
            vit_model, vit_loader, device, "ViT"
        )
        ...
        vit_metrics = {
            ...
            "roc_auc": roc_auc_score(y_true, vit_probs)
            if len(np.unique(y_true)) > 1
            else np.nan,
        }
        ...
        print_ensemble_results(
            ensemble_metrics, cnn_metrics, vit_metrics, y_true, ensemble_preds
        )

        # Save results
        save_ensemble_results(
            y_true, cnn_probs, vit_probs, ensemble_probs, ensemble_metrics, output_dir
        )
