...
def calculate_metrics(
    ...
    try:
        if len(np.unique(y_true)) > 1:
            ...
            logger.warning(
                "Only one class present in true labels, ROC AUC not calculated"
            )
...
def save_predictions(
    y_true: np.ndarray, y_prob: np.ndarray, y_pred: np.ndarray, output_path: Path
) -> None:
    """
...
def main():
    ...
    # Required arguments
    parser.add_argument(
        "--weights", type=str, required=True, help="Path to trained model weights"
    )
    # Get available models as flat list of strings
    models = list_available_models()
    choices = [*models.get("single_models", []), *models.get("physics_models", [])]
    parser.add_argument(
        "--arch", type=str, required=True, choices=choices, help="Model architecture"
    )
    ...
    try:
        ...
        if args.save_predictions:
            ...
            save_predictions(
                y_true, y_prob, y_pred, output_dir / "detailed_predictions.csv"
            )
