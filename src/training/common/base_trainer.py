...
class BaseTrainer(ABC):
    ...
    def create_model(self) -> nn.Module:
        ...
        if hasattr(self.args, "arch") and self.args.arch in [
            "trans_enc_s",
            "light_transformer",
        ]:
        ...
        if not hasattr(self.args, "img_size") or self.args.img_size is None:
            ...
            logger.info(
                f"Auto-detected image size for {getattr(self.args, 'arch', 'resnet18')}: {self.args.img_size}"
            )
    ...
    def setup_training(self) -> None:
        ...
        if scheduler_type == "plateau":
            ...
            self.scheduler = CosineAnnealingLR(
                self.optimizer, T_max=getattr(self.args, "epochs", 20)
            )
    ...
    def save_checkpoint(
        self, checkpoint_dir: Path, epoch: int, is_best: bool = False
    ) -> None:
        ...
        if epoch % 10 == 0 or is_best:
            history_filename = (
                f"training_history_{arch}_amp.json"
                if use_amp
                else f"training_history_{arch}.json"
            )
            self.history.update(
                {
                    "architecture": arch,
    ...
    def check_early_stopping(self, val_loss: float) -> bool:
        ...
        if val_loss < self.best_val_loss - min_delta:
            ...
            logger.info(
                f"No improvement for {self.patience_counter} epochs (patience: {patience})"
            )
    ...
    def train(self) -> None:
        ...
        try:
            ...
            for epoch in range(1, epochs + 1):
                ...
                is_best = val_loss < self.best_val_loss - getattr(
                    self.args, "min_delta", 1e-4
                )
                ...
                self.log_epoch(
                    epoch, epochs, train_loss, train_acc, val_loss, val_acc, epoch_time
                )
            ...
            logger.info(
                f"Final test results: loss={self.test_loss:.4f}, accuracy={self.test_acc:.3f}"
            )
...
def create_base_argument_parser(
    description: str = "Train lens classifier",
