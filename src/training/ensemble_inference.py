...
class ParallelEnsembleInference:
    ...
    def predict_single_model(
        self, model_name: str, dataloader: DataLoader, mc_samples: int = 1
    ...
    def predict_parallel(
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            future_to_model = {
                executor.submit(
                    self.predict_single_model, name, dataloader, mc_samples
                ): name
            ...
            for future in as_completed(future_to_model):
                ...
                try:
                    ...
                    logger.info(
                        f"Completed inference for {model_name}: {logits.shape[0]} samples"
                    )
    ...
    def predict_batch_parallel(
        self, dataloader: DataLoader, mc_samples: int = 1
        ...
        for name, logits_list in results.items():
            ...
            logger.info(
                f"Completed inference for {name}: {final_results[name].shape[0]} samples"
            )
...
class BatchEnsembleProcessor:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized batch processor: batch_size={batch_size}, amp={use_amp}"
        )
    ...
    def process_dataset(
        ...
        logger.info(
            f"Completed processing: {total_samples} samples in {total_time:.1f}s "
...
def benchmark_inference_speed(
    models: Dict[str, nn.Module], dataloader: DataLoader, num_runs: int = 3
) -> Dict[str, float]:
    """
...
def main():
    ...
    parser.add_argument(
        "--img-size", type=int, default=None, help="Image size for preprocessing"
    )
    parser.add_argument(
    ...
    # Inference arguments
    parser.add_argument(
        "--mc-samples", type=int, default=10, help="Number of MC dropout samples"
    )
    parser.add_argument(
        "--amp", action="store_true", help="Use automatic mixed precision"
    )
    parser.add_argument(
        "--parallel", action="store_true", help="Use parallel model execution"
    )
    parser.add_argument(
        "--batch-parallel",
        action="store_true",
    ...
    parser.add_argument(
        "--save-predictions", action="store_true", help="Save predictions to file"
    )
    ...
    logger.info(
        f"Created optimized test data loader with {len(test_loader.dataset)} samples"
    )
    ...
    if args.parallel and len(models) > 1:
        ...
        if args.batch_parallel:
            logger.info(
                "Using optimized batch-parallel inference for optimal performance..."
            )
