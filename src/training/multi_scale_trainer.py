...
def _materialize_scale_from_base(batch, scale, device, tfm_cache):
    ...
    if scale not in tfm_cache:
        # Use astronomy default normalization (zero-mean, unit-variance)
        # Replace with survey-specific stats when available
...
class MultiScaleDataset(Dataset):
    ...
    def __init__(
        ...
        if memory_efficient:
            logger.info(
                f"Memory-efficient multi-scale dataset: scales={scales}, augment={augment}"
            )
        else:
            logger.info(
                f"Standard multi-scale dataset: scales={scales}, augment={augment}"
            )
        ...
        for scale in self.scales:
            ...
            if self.augment:
                if scale >= 128:
                    ...
                    aug = [T.RandomHorizontalFlip(0.5), T.ColorJitter(0.1, 0.1)]
    ...
    def get_scale_image(self, base_image, scale: int) -> torch.Tensor:
        ...
        if scale not in self.scales:
            raise ValueError(
                f"Scale {scale} not available. Available scales: {self.scales}"
            )
...
class ProgressiveMultiScaleTrainer:
    ...
    def should_advance_scale(self) -> bool:
        ...
        return (
            self.epoch_count > 0
            and self.epoch_count % self.scale_epochs == 0
            and self.current_scale_idx < len(self.scales) - 1
        )
    ...
    def train_epoch(
        ...
        for batch in dataloader:
            ...
            images = _materialize_scale_from_base(
                batch, current_scale, self.device, tfm_cache
            )
            bs = labels.size(0)
    ...
    def validate_epoch(
        self, dataloader: DataLoader, criterion: nn.Module, use_amp: bool = False
    ) -> Dict[str, float]:
        """
...
class MultiScaleTrainer:
    ...
    def __init__(
        ...
        logger.info(
            f"Multi-scale trainer: scales={scales}, consistency_weight={consistency_weight}"
        )
    ...
    def _group_scales_by_memory(self) -> List[List[int]]:
        ...
        for scale, memory_usage in sorted_scales:
            if (
                current_memory + memory_usage <= max_memory_per_group
        ...
        for batch in dataloader:
            ...
            if "base_image" in batch:
                ...
                for scale in self.scales:
                    ...
                    images = torch.stack(scaled_images).to(
                ...
                for scale_group in scale_groups:
                    ...
                    for scale in scale_group:
                        images = batch[f"image_{scale}"].to(
                            self.device, non_blocking=True
                        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Multi-scale training for lens classification"
    )
    ...
    parser.add_argument(
        "--progressive", action="store_true", help="Use progressive training"
    )
    parser.add_argument(
        "--scale-epochs",
        type=int,
    ...
    val_multiscale = MultiScaleDataset(
        val_base, scales, augment=False, memory_efficient=True
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_metrics = trainer.train_epoch(
            train_loader, optimizer, train_criterion, args.amp
        )
