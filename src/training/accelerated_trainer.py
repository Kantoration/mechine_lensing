            ...
            if total_training_time > 0:
                if self.total_samples_processed > 0:
                    stats["samples_per_second"] = (
                        self.total_samples_processed / total_training_time
                    )
                else:
                    # Fallback to epochs per second if no sample count available
                    stats["samples_per_second"] = (
                        len(self.epoch_times) / total_training_time
                    )
                ...
                if self.total_batches_processed > 0:
                    stats["batches_per_second"] = (
                        self.total_batches_processed / total_training_time
                    )
...
def set_seed(seed: int = 42) -> None:
    ...
    logger.info(
        f"Set random seed to {seed}, deterministic={os.getenv('DETERMINISTIC', 'false')}"
    )
    ...
    try:
        ...
        if args.benchmark:
            ...
            benchmark_suite = BenchmarkSuite(
                output_dir=str(checkpoint_dir / "benchmarks")
            )
        ...
        logger.info(
            f"Starting accelerated training for {args.epochs} epochs (patience: {args.patience}, min_delta: {args.min_delta})"
        )
        logger.info(
            f"Architecture: {args.arch}, Batch size: {args.batch_size}, "
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            # Train and validate
            train_loss, train_acc, samples_processed, batches_processed = (
                train_epoch_amp(
                    model,
                    train_loader,
                    criterion,
            ...
            if val_loss < best_val_loss - args.min_delta:
                ...
                model_filename = (
                    f"best_{args.arch}_amp.pt" if use_amp else f"best_{args.arch}.pt"
                )
        ...
        history_filename = (
            f"training_history_{args.arch}_amp.json"
            if use_amp
            else f"training_history_{args.arch}.json"
        )
