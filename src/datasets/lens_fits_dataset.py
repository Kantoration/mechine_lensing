...
class LensFITSDataset(Dataset):
    ...
    def __init__(
        ...
        if 'filepath' not in self.df.columns:
            raise ValueError(f"CSV must contain 'filepath' column")
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
            ...
            for map_name, hdu_idx in self.map_hdus.items():
                if hdu_idx < len(hdul):
                    ...
                    if map_data is not None:
                        ...
                        map_data = np.nan_to_num(map_data, nan=0.0, posinf=0.0, neginf=0.0)
                        ...
                        if map_name == 'alpha':
                            ...
                            if map_data.ndim == 2:
                                ...
                                if map_data.shape[-1] == 2:
                                    ...
                                    raise ValueError(f"Alpha map shape unclear: {map_data.shape}")
        ...
        pixel_scale_arcsec = float(
            row.get('pixel_scale_arcsec',
            header.get('PIXSCALE', 0.1))
        )
        pixel_scale_y_arcsec = float(
            row.get('pixel_scale_y_arcsec', pixel_scale_arcsec)
        )
        ...
        sample['meta'] = {
            ...
            'dx': dx,
            'dy': dy,
            'z_l': row.get('z_l', None) if 'z_l' in row and pd.notna(row.get('z_l')) else None,
            'z_s': row.get('z_s', None) if 'z_s' in row and pd.notna(row.get('z_s')) else None,
            'sigma_crit': float(row.get('sigma_crit', 1.0)) if 'sigma_crit' in row and pd.notna(row.get('sigma_crit')) else 1.0,
