...
from .ensemble.registry import (
    make_model as make_ensemble_model,
    get_model_info as ensemble_get_model_info,
)
from .ensemble.physics_informed_ensemble import PhysicsInformedEnsemble
...
class ModelConfig:
    ...
    ensemble_strategy: str = (
        "uncertainty_weighted"  # "uncertainty_weighted", "physics_informed"
    )
    ...
    def _validate_config(self) -> None:
        ...
        if self.model_type not in ["single", "ensemble", "physics_informed"]:
            raise ValueError(
                f"Invalid model_type: {self.model_type}. Must be 'single', 'ensemble', or 'physics_informed'"
            )
        ...
        if self.model_type != "single":
            ...
            if len(self.architectures) < 2:
                raise ValueError(
                    f"{self.model_type} model requires at least 2 architectures"
                )

        # Validate ensemble strategy
        if self.model_type == "ensemble" and self.ensemble_strategy not in [
        ...
        if not 0 <= self.physics_weight <= 1:
            raise ValueError(
                f"physics_weight must be in [0, 1], got: {self.physics_weight}"
            )
...
class UnifiedModelFactory:
    ...
    def _create_single_model(self, config: ModelConfig) -> nn.Module:
        ...
        if config.bands not in [1, 3]:
            logger.warning(
                f"Unusual number of bands: {config.bands}. Expected 1 (grayscale) or 3 (RGB)"
            )
        ...
        try:
            ...
            if architecture in [
                ...
                try:
                    ...
                    logger.debug(
                        f"Successfully created enhanced model backbone: {architecture}"
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to create enhanced model backbone '{architecture}': {e}"
                    )
                    raise ValueError(
                        f"Enhanced model backbone creation failed for '{architecture}': {e}"
                    ) from e
                ...
                try:
                    ...
                    logger.info(
                        "Created LensGNN (graph-based). Ensure graph_builder provides node_dim at runtime."
                    )
                except Exception as e:
                ...
                try:
                    ...
                    model = nn.Sequential(backbone, head)
                    logger.debug(
                        f"Successfully created standard model backbone: {architecture}"
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to create standard model backbone '{architecture}': {e}"
                    )
                    raise ValueError(
                        f"Standard model backbone creation failed for '{architecture}': {e}"
                    ) from e
            ...
            try:
                ...
                logger.debug(
                    f"Successfully applied performance optimizations to {architecture}"
                )
            except Exception as e:
                logger.warning(
                    f"Failed to apply performance optimizations to '{architecture}': {e}"
                )
            ...
            try:
                ...
                if self.model_registry.get(architecture, {}).get(
                    "supports_physics", False
                ):
                    logger.debug(
                        f"Successfully applied physics wrapper to {architecture}"
                    )
            except Exception as e:
                logger.warning(
                    f"Failed to apply physics wrapper to '{architecture}': {e}"
                )
            ...
            raise ImportError(
                f"Cannot create model '{architecture}' due to missing dependencies: {e}"
            ) from e
        except (ValueError, TypeError) as e:
            logger.error(
                f"Configuration error while creating model '{architecture}': {e}"
            )
            raise ValueError(
                f"Invalid configuration for model '{architecture}': {e}"
            ) from e
        except (MemoryError, RuntimeError) as e:
            logger.error(f"Resource error while creating model '{architecture}': {e}")
            raise RuntimeError(
                f"Insufficient resources to create model '{architecture}': {e}"
            ) from e
        except Exception as e:
            logger.error(f"Unexpected error while creating model '{architecture}': {e}")
            raise RuntimeError(
                f"Failed to create model '{architecture}' due to unexpected error: {e}"
            ) from e
    ...
    def _create_ensemble_model(self, config: ModelConfig) -> nn.Module:
        ...
        try:
            ...
            if not config.architectures or len(config.architectures) < 2:
                raise ValueError(
                    f"Ensemble requires at least 2 architectures, got: {config.architectures}"
                )
            ...
            for arch in config.architectures:
                ...
                if arch not in self.model_registry and arch not in [
                    "enhanced_light_transformer_arc_aware",
                    "enhanced_light_transformer_multi_scale",
                    "enhanced_light_transformer_adaptive",
                ]:
            ...
            if successful_members < 2:
                raise ValueError(
                    f"Ensemble requires at least 2 valid architectures. "
            ...
            try:
                if config.ensemble_strategy == "uncertainty_weighted":
                    ...
                    logger.debug(
                        f"Successfully created uncertainty-weighted ensemble with {len(member_configs)} members"
                    )
                    ...
                    logger.debug(
                        f"Successfully created physics-informed ensemble with {len(member_configs)} members"
                    )
                else:
                    raise ValueError(
                        f"Unknown ensemble strategy: {config.ensemble_strategy}"
                    )

                logger.info(
                    f"Successfully created ensemble model with {len(config.architectures)} members "
                ...
                raise ImportError(
                    f"Cannot create ensemble due to missing dependencies: {e}"
                ) from e
            except Exception as e:
                logger.error(f"Error during ensemble creation: {e}")
                raise ValueError(
                    f"Failed to create ensemble with strategy '{config.ensemble_strategy}': {e}"
                ) from e

        except ImportError as e:
            logger.error(f"Import error while creating ensemble model: {e}")
            raise ImportError(
                f"Cannot create ensemble model due to missing dependencies: {e}"
            ) from e
        except (ValueError, TypeError) as e:
            ...
            raise RuntimeError(
                f"Insufficient resources to create ensemble model: {e}"
            ) from e
        except Exception as e:
            logger.error(f"Unexpected error while creating ensemble model: {e}")
            raise RuntimeError(
                f"Failed to create ensemble model due to unexpected error: {e}"
            ) from e
    ...
    def _create_physics_informed_model(self, config: ModelConfig) -> nn.Module:
        ...
        if not config.architectures or len(config.architectures) < 2:
            raise ValueError(
                f"Physics-informed ensemble requires at least 2 architectures, got: {config.architectures}"
            )
        ...
        try:
            ...
            logger.info(
                f"Created physics-informed ensemble with {len(config.architectures)} members "
    ...
    def _apply_performance_optimizations(
        self, model: nn.Module, config: ModelConfig
    ) -> nn.Module:
    ...
    def _verify_logits_output(self, model: nn.Module, config: ModelConfig) -> None:
        ...
        try:
            ...
            if expected_outputs != "logits":
                raise ValueError(
                    f"Head for '{config.architecture}' must output logits, got: {expected_outputs}"
                )
            ...
            if isinstance(output, torch.Tensor):
                ...
                if torch.all((output > 0) & (output < 1)) and output.std() < 0.25:
                    raise ValueError(
                        "Model appears to output probabilities; expected logits."
                    )

                logger.debug(
                    f"Model output verified as logits (range: [{output.min():.3f}, {output.max():.3f}])"
                )
    ...
    def benchmark_model_creation(
        self, config: ModelConfig, num_runs: int = 5
    ) -> Dict[str, float]:
...
def build_model(
    arch: str, pretrained: bool = True, dropout_rate: float = 0.2
) -> nn.Module:
...
def make_model(
    name: str, bands: int = 3, pretrained: bool = True, dropout_p: float = 0.2
