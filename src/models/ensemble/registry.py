...
MODEL_REGISTRY: Dict[str, Dict[str, Any]] = {
    ...
    "light_transformer": {
        ...
        "backbone_kwargs": {"max_tokens": 256},
        "feature_dim": 256,
        "input_size": 112,
    ...
    "enhanced_light_transformer_multi_scale": {
        ...
        "backbone_kwargs": {
            ...
            "attention_config": {"scales": [1, 2, 4], "fusion_method": "weighted_sum"},
        },
        "feature_dim": 256,
        "input_size": 112,
    ...
    "enhanced_light_transformer_adaptive": {
        ...
        "backbone_kwargs": {
            ...
            "attention_config": {"adaptation_layers": 2},
        },
        "feature_dim": 256,
        "input_size": 112,
...
def make_model(
    ...
    backbone_kwargs.update({"in_ch": len(contract.bands), "pretrained": pretrained})
    ...
    logger.info(
        f"Created model pair: {name} with bands={contract.bands}, "
...
def create_ensemble_members(
    ...
    for arch in architectures:
        backbone, head, _ = make_model(
            name=arch, bands=bands, pretrained=pretrained, dropout_p=dropout_p
        )
...
def create_resnet_vit_ensemble(
    bands: int = 3, pretrained: bool = True
) -> list[Tuple[nn.Module, nn.Module]]:
    """Create a ResNet-18 + ViT-B/16 ensemble."""
    return create_ensemble_members(
        ["resnet18", "vit_b_16"], bands=bands, pretrained=pretrained
    )


def create_resnet_ensemble(
    bands: int = 3, pretrained: bool = True
) -> list[Tuple[nn.Module, nn.Module]]:
    """Create a ResNet-18 + ResNet-34 ensemble."""
    return create_ensemble_members(
        ["resnet18", "resnet34"], bands=bands, pretrained=pretrained
    )


def create_physics_informed_ensemble(
    bands: int = 3, pretrained: bool = True
) -> list[Tuple[nn.Module, nn.Module]]:
