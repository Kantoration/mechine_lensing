...
class UncertaintyWeightedEnsemble(nn.Module):
    ...
    def __init__(
        ...
        logger.info(
            f"Created uncertainty-weighted ensemble with {self.num_members} members: "
            f"{self.member_names}"
        )
    ...
    def mc_predict(
        ...
        logits_tensor = torch.stack(
            member_means, dim=0
        )  # [num_members, batch_size] - LOGITS
        vars_tensor = torch.stack(
            member_vars, dim=0
        )  # [num_members, batch_size] - LOGIT VARIANCES
    ...
    def analyze_member_contributions(
        ...
        (
            ...
            individual_preds,
            individual_vars,
        ) = self.mc_predict(inputs, mc_samples, return_individual=True)
...
def create_uncertainty_weighted_ensemble(
    ...
    return UncertaintyWeightedEnsemble(
        members=members, member_names=architectures, temperatures=temperatures
    )
