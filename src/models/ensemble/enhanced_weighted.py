...
class EnhancedUncertaintyEnsemble(nn.Module):
    ...
    def __init__(
        ...
        for i, config in enumerate(member_configs):
            ...
            backbone, head, feature_dim = make_model(
                name=name, bands=bands, pretrained=pretrained, dropout_p=dropout_p
            )
        ...
        if learnable_trust:
            ...
            self.member_trust = nn.Parameter(
                torch.tensor(trust_values, dtype=torch.float32), requires_grad=True
            )
            ...
            # Fixed trust values
            trust_values = [
                config.get("temperature", initial_trust) for config in member_configs
            ]
            self.register_buffer(
                "member_trust", torch.tensor(trust_values, dtype=torch.float32)
            )

        logger.info(
            f"Created enhanced ensemble with {len(self.members)} members, "
            f"learnable_trust={learnable_trust}"
        )

    def _run_mc_dropout(
        self, model: nn.Module, x: torch.Tensor, mc_samples: int, member_name: str
        ...
        try:
            ...
            logits_stack = torch.stack(
                logits_samples, dim=0
            )  # [mc_samples, batch_size]

            return {"logits_samples": logits_stack, "has_aleatoric": has_aleatoric}
    ...
    def analyze_member_contributions(
        self, inputs: Dict[str, torch.Tensor], mc_samples: int = 20
    ) -> Dict[str, Any]:
        """
        ...
        for name in self.member_names:
            ...
            decomp = {
                "epistemic_uncertainty": member_data["epistemic_variance"]
                .mean()
                .item(),
            ...
            if "aleatoric_variance" in member_data:
                decomp["aleatoric_uncertainty"] = (
                    member_data["aleatoric_variance"].mean().item()
                )
                decomp["epistemic_fraction"] = (
                    decomp["epistemic_uncertainty"] / decomp["total_uncertainty"]
                )
                decomp["aleatoric_fraction"] = (
                    decomp["aleatoric_uncertainty"] / decomp["total_uncertainty"]
                )
    ...
    def get_trust_parameters(self) -> Dict[str, float]:
        ...
        return {
            name: trust.item()
            for name, trust in zip(self.member_names, self.member_trust)
        }
...
def create_enhanced_ensemble(
    member_configs: List[Dict[str, Any]], learnable_trust: bool = True, **kwargs
    ...
    return EnhancedUncertaintyEnsemble(
        member_configs=member_configs, learnable_trust=learnable_trust, **kwargs
    )


def create_three_member_ensemble(
    bands: int = 3, use_aleatoric: bool = True, pretrained: bool = True
) -> EnhancedUncertaintyEnsemble:
    """
    ...
    return create_enhanced_ensemble(
        member_configs=member_configs, learnable_trust=True, initial_trust=1.0
    )
