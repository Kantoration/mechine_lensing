...
class _SequentialMember(nn.Module):
    """Wrap backbone/head pairs so we can attach contracts and enable MC-dropout."""
...
class PhysicsInformedEnsemble(nn.Module):
    ...
    def __init__(
        ...
        self.member_contracts: Dict[str, ModelContract] = {}
        self.member_feature_dims: Dict[str, int] = {}

        for i, config in enumerate(member_configs):
            name = config["name"]
            bands_cfg = config.get("bands", 3)
            bands_list = config.get("bands_list")
            if isinstance(bands_cfg, (list, tuple)):
                bands_list = list(bands_cfg)
                bands = len(bands_list)
            else:
                bands = int(bands_cfg)
            if bands_list is not None:
                bands_list = [str(b) for b in bands_list]
            ...
            backbone, head, feature_dim, contract = make_model(
                ...
                dx=config.get("dx"),
                dy=config.get("dy"),
                sigma_crit_policy=config.get("sigma_crit_policy", "dimensionless"),
                task_type=config.get("task_type", "classification"),
                input_type=config.get("input_type", "image"),
            )

            # Combine into single model when backbone/head pair is returned
            if head is not None or feature_dim:
                model = _SequentialMember(backbone, head)
            else:
                model = backbone

            # Propagate contract to wrapper for downstream consumers
            model.contract = contract  # type: ignore[attr-defined]
            ...
            self.member_has_physics[name] = physics_flag

            logger.info(
                f"Added ensemble member {i + 1}/{len(member_configs)}: {name} "
        ...
        if uncertainty_estimation:
            ...
            self.physics_weighting_net = nn.Sequential(
                nn.Linear(
                    len(member_configs) * 3, 64
                ),  # logits + uncertainties + physics features
                nn.ReLU(),
                nn.Linear(64, 32),
                nn.ReLU(),
        ...
        logger.info(
            f"Created physics-informed ensemble with {len(member_configs)} members"
        )

    def _validate_contract_compatibility(self) -> None:
        """Ensure all members agree on band ordering, input, and task type."""
        if not self.member_contracts:
            return
        contracts = list(self.member_contracts.items())
        _, reference = contracts[0]
        ref_bands = tuple(reference.bands)
        ref_task = reference.task_type
        ref_input = reference.input_type
        ref_sigma_policy = reference.sigma_crit_policy

        for name, contract in contracts[1:]:
            if tuple(contract.bands) != ref_bands:
                raise ValueError(
                    f"Ensemble band mismatch for member '{name}': {contract.bands} vs {ref_bands}"
                )
            if contract.task_type != ref_task:
                raise ValueError(
                    f"Ensemble task_type mismatch for member '{name}': {contract.task_type} vs {ref_task}"
                )
            if contract.input_type != ref_input:
                raise ValueError(
                    f"Ensemble input_type mismatch for member '{name}': {contract.input_type} vs {ref_input}"
                )
            if (
                ref_sigma_policy is not None
                and contract.sigma_crit_policy != ref_sigma_policy
            ):
                raise ValueError(
                    f"Ensemble sigma_crit_policy mismatch for member '{name}': "
                    f"{contract.sigma_crit_policy} vs {ref_sigma_policy}"
                )
    ...
    def _resize_maps(
        ...
        if "psi" in sample:
            ...
            if contract.dx is not None and contract.dy is not None:
                ...
                H_old, W_old = (
                    psi.shape[-2:] if psi.ndim == 3 else sample["psi"].shape[-2:]
                )
                ...
                out["alpha_from_psi"] = torch.cat([gx, gy], dim=1).squeeze(
                    0
                )  # [2, H, W]
    ...
    def forward(self, inputs: Dict[str, torch.Tensor]) -> Dict[str, Any]:
        ...
        resized_cache: Dict[
            Tuple[str, int, int, Tuple[int, int, int, int]], torch.Tensor
        ] = {}
        ...
        for i, (name, model) in enumerate(zip(self.member_names, self.members)):
            ...
            if name not in inputs:
                raise KeyError(
                    f"Missing input for ensemble member '{name}'. "
                    f"Available inputs: {list(inputs.keys())}. "
                    f"Ensure all ensemble members have corresponding inputs."
                )

            x = inputs[name]
            contract = self.member_contracts.get(name)
            if not isinstance(x, torch.Tensor):
                raise TypeError(
                    f"Ensemble input for '{name}' must be a torch.Tensor, got {type(x)}"
                )
            if contract is not None and x.shape[1] != len(contract.bands):
                raise ValueError(
                    f"Input channel mismatch for '{name}': expected {len(contract.bands)} channels "
                    f"from contract {contract.bands}, received {x.shape[1]}"
                )
            if x.ndim != 4:
                raise ValueError(
                    f"Expected 4D input tensor [B,C,H,W] for '{name}', got shape {tuple(x.shape)}"
                )
            ...
            if self.member_has_physics[name]:
                ...
                if loss_val is None:
                    ...
                    loss_tensor = torch.as_tensor(
                        loss_val, device=device, dtype=torch.float32
                    )
                ...
                if loss_tensor.dim() == 0:
                    ...
                    raise ValueError(
                        f"physics_reg_loss must be scalar or shape [B], got {loss_tensor.shape}"
                    )
            ...
            if self.uncertainty_estimation:
                logit_uncertainty = self._estimate_uncertainty_logits(
                    model, x, num_samples=self.mc_samples
                )
        ...
        if self.uncertainty_estimation:
            weights = self._compute_physics_weights_logits(
                logits, uncertainties, member_physics_losses
            )
        else:
            weights = torch.ones(batch_size, len(self.members), device=device) / len(
                self.members
            )
        ...
        total_physics_loss = (
            member_physics_losses.mean(dim=0).sum() * self.physics_weight
        )
        ...
        output = {
            ...
            "member_predictions": torch.sigmoid(logits).clamp(
                1e-6, 1 - 1e-6
                        ...
                        try:
                            ...
                            if hasattr(block.attention, "get_physics_info"):
                                ...
                                if "physics_reg_loss" in block_physics_info:
                                    physics_reg_loss += block_physics_info[
                                        "physics_reg_loss"
                                    ]
                                if "attention_maps" in block_physics_info:
                                    attention_maps[f"block_{i}"] = block_physics_info[
                                        "attention_maps"
                                    ]
                        except Exception as e:
                            logger.debug(
                                f"Could not extract physics info from block {i}: {e}"
                            )
    ...
    def _forward_physics_logits(
        self, model: nn.Module, x: torch.Tensor
    ) -> Tuple[torch.Tensor, PhysicsInfo]:
        """
        ...
        if hasattr(model, "__getitem__") and len(model) >= 1:
            ...
            if hasattr(backbone, "transformer_blocks"):
                ...
                for i, block in enumerate(getattr(backbone, "transformer_blocks", [])):
                    if hasattr(block, "attention") and hasattr(
                        block.attention, "get_physics_info"
                    ):
                        try:
    ...
    def _estimate_uncertainty_logits(
        self, model: nn.Module, x: torch.Tensor, num_samples: int = 10
    ) -> torch.Tensor:
        """
        ...
        logit_uncertainty = torch.std(
            logit_samples, dim=0, unbiased=False
        )  # [batch_size]
    ...
    def _compute_physics_weights(
        self, predictions: torch.Tensor, uncertainties: torch.Tensor
    ) -> torch.Tensor:
        """Compute physics-aware ensemble weights."""
        if hasattr(self, "physics_weighting_net"):
            # Combine predictions and uncertainties
            features = torch.cat(
                [predictions, uncertainties], dim=-1
            )  # [B, 2*num_members]
            ...
            weights = inv_uncertainties / torch.sum(
                inv_uncertainties, dim=1, keepdim=True
            )
    ...
    def _compute_physics_weights_logits(
        ...
        if hasattr(self, "physics_weighting_net"):
            ...
            features = torch.cat(
                [logits, u_clamped, physics_losses], dim=-1
            )  # [B, 3*M]
    ...
    def _compute_physics_consistency(
        ...
        predictions = output[
            "member_predictions"
        ]  # These are probabilities for consistency analysis
    ...
    def _safe_correlation(
        self, a: torch.Tensor, b: torch.Tensor, eps: float = 1e-8
    ) -> float:
        """
...
def create_physics_informed_ensemble_from_config(
    config_path: str,
