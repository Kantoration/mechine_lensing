...
class PhysicsRegularizedAttention(nn.Module):
    ...
    def _create_physics_constraints(self) -> Dict[str, torch.Tensor]:
        ...
        constraints["tangential"] = torch.tensor(
            [[[[-1, 0, -1], [0, 4, 0], [-1, 0, -1]]]], dtype=torch.float32
        )
    ...
    def _compute_physics_attention_prior(
        ...
        attention_prior = torch.bmm(
            attention_map.unsqueeze(2), attention_map.unsqueeze(1)
        )  # [B, N, N]
    ...
    def forward(
        ...
        if H is not None and W is not None and H * W == N:
            ...
            physics_prior = physics_prior.unsqueeze(1).expand(
...
class AdaptivePhysicsAttention(nn.Module):
    ...
    def __init__(
        self, embed_dim: int = 256, num_heads: int = 4, adaptation_layers: int = 2
    ):
        """
...
def analyze_kernel_evolution(
    model: PhysicsRegularizedAttention, save_path: Optional[str] = None
    ...
    for i, kernel in enumerate(model.kernels):
        ...
        if kernel_type in model.physics_constraints:
            ...
            alignment = F.cosine_similarity(
                kernel.flatten(), constraint_resized.flatten(), dim=0
            ).item()
    ...
    if save_path:
        ...
        fig, axes = plt.subplots(
            2, len(model.kernels), figsize=(4 * len(model.kernels), 8)
        )
        ...
        for i, kernel in enumerate(model.kernels):
            ...
            if kernel_type in model.physics_constraints:
                ...
                axes[1, i].imshow(
                    constraint_resized.detach().cpu().numpy()[0, 0], cmap="RdBu_r"
                )
