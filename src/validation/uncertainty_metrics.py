...
class UncertaintyValidator:
    ...
    def _compute_reliability_diagram_metrics(
        ...
        # Reliability metrics
        metrics["reliability_mse"] = np.mean(
            (fraction_of_positives - mean_predicted_value) ** 2
        )
        metrics["reliability_mae"] = np.mean(
    ...
    def _compute_sharpness_metrics(self, uncertainties: np.ndarray) -> Dict[str, float]:
        ...
        metrics["uncertainty_entropy"] = self._compute_uncertainty_entropy(
            uncertainties
        )
    ...
    def _compute_reliability_metrics(
        ...
        conf_weighted_acc = np.average(
            pred_labels == ground_truth, weights=conf_weights
        )
        ...
        if high_conf_mask.sum() > 0:
            high_conf_acc = np.mean(
                pred_labels[high_conf_mask] == ground_truth[high_conf_mask]
            )
        ...
        if low_conf_mask.sum() > 0:
            low_conf_acc = np.mean(
                pred_labels[low_conf_mask] == ground_truth[low_conf_mask]
            )
    ...
    def _fit_temperature_scaling(
        ...
        def temperature_loss(temp):
            ...
            loss = -np.mean(
                ground_truth * np.log(scaled_probs + 1e-8)
    ...
    def _apply_temperature_scaling(
        self, predictions: np.ndarray, temperature: float
    ) -> np.ndarray:
...
def create_uncertainty_validation_report(
    ...
    calibration_keys = [
        ...
        if k.startswith(("ece", "mce", "brier_score"))
    ]
    ...
    sharpness_keys = [
        k
        for k in validation_results.keys()
        if k.startswith(("mean_uncertainty", "sharpness"))
    ]
    ...
    reliability_keys = [
        k
        for k in validation_results.keys()
        if k.startswith(("accuracy", "confidence_weighted"))
    ]
    ...
    separation_keys = [
        k
        for k in validation_results.keys()
        if k.startswith(("epistemic_", "aleatoric_"))
    ]
