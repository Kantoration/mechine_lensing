...
class SourceReconstructor:
    ...
    def __init__(
        ...
        logger.info(
            f"Source reconstructor initialized with pixel scales: "
    ...
    def _linear_inversion(
        self, lensed_image: np.ndarray, source_size: Tuple[int, int]
    ) -> np.ndarray:
        """
        ...
        try:
            ...
            source_flat, _, _, _ = np.linalg.lstsq(
                lensing_matrix, image_flat, rcond=None
            )
    ...
    def _create_lensing_matrix(
        self, H_img: int, W_img: int, H_src: int, W_src: int
    ) -> np.ndarray:
        """
...
class SourceQualityValidator:
    ...
    def _validate_flux_conservation(
        self, source: np.ndarray, lensed_image: np.ndarray, lens_model: Any
    ) -> float:
        """
    ...
    def _compute_morphology_similarity(
        self, source1: np.ndarray, source2: np.ndarray
    ) -> float:
        """
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            if "reconstructed_source" in source_info:
                all_reconstructed_sources.append(
                    source_info["reconstructed_source"].cpu()
                )
...
def create_source_validation_report(
    ...
    report.append("-" * 50)
    recon_keys = [
        k for k in validation_results.keys() if k.startswith("source_reconstruction_")
    ]
    ...
    report.append("-" * 50)
    physicality_keys = [
        k for k in validation_results.keys() if k.startswith("source_physicality")
    ]
    ...
    quality_keys = [
        ...
        if k.startswith(("source_smoothness", "source_flux_conservation"))
    ]
    ...
    stat_keys = [
        k
        for k in validation_results.keys()
        if k.startswith(("source_chi_squared", "source_bayesian_evidence"))
    ]
    ...
    report.append("-" * 50)
    cross_band_keys = [
        k for k in validation_results.keys() if k.startswith("cross_band_")
    ]
    ...
    if score_components:
        ...
        if overall_score < 0.5:
            ...
            report.append(
                "  - Consider improving regularization or reconstruction method"
            )
