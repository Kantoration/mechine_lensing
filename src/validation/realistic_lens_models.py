    ...
    def _nfw_deflection_magnitude(self, s: np.ndarray) -> np.ndarray:
        ...
        if np.any(mask1):
            ...
            alpha_mag[mask1] = (
                ...
                * (
                    1
                    - 2 / np.sqrt(1 - s1**2) * np.arctanh(np.sqrt((1 - s1) / (1 + s1)))
                )
            )
        ...
        if np.any(mask3):
            ...
            alpha_mag[mask3] = (
                ...
                * self.norm_factor
                / (s3**2 - 1)
                * (1 - 2 / np.sqrt(s3**2 - 1) * np.arctan(np.sqrt((s3 - 1) / (s3 + 1))))
            )
...
class CompositeLensModel:
    ...
    def __init__(
        self, components: List[Any], external_shear: Tuple[float, float] = (0.0, 0.0)
    ):
        """
    ...
    def critical_curve(self, resolution: int = 100) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if len(contours) > 0:
            ...
            x_crit = x_range[0] + largest_contour[:, 1] * (x_range[-1] - x_range[0]) / (
                resolution - 1
            )
            y_crit = y_range[0] + largest_contour[:, 0] * (y_range[-1] - y_range[0]) / (
                resolution - 1
            )
        else:
...
class RealisticLensValidator:
    ...
    def validate_einstein_radius_realistic(
        ...
        for i in range(attn_np.shape[0]):
            ...
            estimated_radius = self._estimate_einstein_radius_realistic(
                attn_map, pixel_scale
            )
            ...
            if estimated_radius is not None and true_radius > 0:
                ...
                metrics["realistic_einstein_radius_relative_error"].append(
                    relative_error
                )

                # Compute critical curve overlap
                overlap = self._compute_critical_curve_overlap(
                    attn_map, lens_model, pixel_scale
                )
                metrics["critical_curve_overlap"].append(overlap)

                # Compute caustic overlap
                caustic_overlap = self._compute_caustic_overlap(
                    attn_map, lens_model, pixel_scale
                )
    ...
    def _estimate_einstein_radius_realistic(
        ...
        r = np.sqrt(
            (x_arcsec - center_x_arcsec) ** 2 + (y_arcsec - center_y_arcsec) ** 2
        )
    ...
    def _compute_critical_curve_overlap(
        self, attention_map: np.ndarray, lens_model: Any, pixel_scale: float
    ) -> float:
        """
        ...
        f_x = interp1d(
            t, x_sorted, kind="cubic", bounds_error=False, fill_value="extrapolate"
        )
        f_y = interp1d(
            t, y_sorted, kind="cubic", bounds_error=False, fill_value="extrapolate"
        )
    ...
    def validate_lensing_equation_realistic(
        ...
        for i in range(attn_np.shape[0]):
            ...
            image_positions = self._find_image_positions_realistic(
                attn_map, pixel_scale
            )
            ...
            if len(image_positions) > 0:
                ...
                if residuals is not None and len(residuals) > 0:
                    ...
                    metrics["realistic_lensing_equation_satisfied"].append(
                        equation_satisfied
                    )
    ...
    def _compute_lensing_equation_residual_realistic(
        ...
        for img_pos in image_positions:
            ...
            alpha_x, alpha_y = lens_model.deflection_angle(img_pos[0], img_pos[1])
