...
class AttentionVisualizer:
    ...
    def visualize_attention_comparison(
        ...
        for i in range(B):
            ...
            im = axes[i, col_idx].imshow(
                attn_map, cmap=self.attention_cmap, vmin=0, vmax=1
            )
            ...
            if ground_truth_maps is not None:
                ...
                im = axes[i, col_idx].imshow(
                    gt_map, cmap=self.attention_cmap, vmin=0, vmax=1
                )
        ...
        for i in range(min(4, attention_maps.shape[0])):
            ...
            if einstein_radii is not None:
                ...
                circle = patches.Circle(
                    center, einstein_radius, fill=False, color="red", linewidth=2
                )
                ax.add_patch(circle)
    ...
    def _plot_physics_metrics(self, ax: plt.Axes, metrics: Dict[str, float]):
        ...
        colors = [
            "red" if v < 0.5 else "orange" if v < 0.7 else "green" for v in values
        ]
    ...
    def _plot_attention_statistics(self, ax: plt.Axes, attention_maps: torch.Tensor):
        ...
        # Create scatter plot
        scatter = ax.scatter(
            mean_attention, std_attention, c=sparsity, s=100, alpha=0.7, cmap="viridis"
        )
    ...
    def _plot_correlation_analysis(
        self, ax: plt.Axes, attention_maps: torch.Tensor, metrics: Dict[str, float]
    ):
...
def create_physics_plots(
    validation_results: Dict[str, float], save_dir: str = "validation_plots"
) -> List[str]:
    """
...
def create_attention_analysis_report(
    ...
    axes[1].scatter(
        attn_np.flatten()[sample_indices],
        gt_np.flatten()[sample_indices],
        alpha=0.5,
        s=1,
    )
    ...
    axes[1].set_title(
        f"Attention vs Ground Truth (r={attention_stats['correlation_with_gt']:.3f})"
    )
    axes[1].grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig(
        os.path.join(save_dir, "attention_summary.png"), dpi=150, bbox_inches="tight"
    )
    plt.close()
