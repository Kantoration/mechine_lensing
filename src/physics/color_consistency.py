...
class ColorAwarePhotometry:
    ...
    def _compute_colors(
        self, fluxes: Dict[str, float], errors: Dict[str, float]
    ) -> Dict[str, float]:
...
class ColorConsistencyPrior:
    ...
    def huber_loss(self, r2: torch.Tensor) -> torch.Tensor:
        ...
        d = self.delta
        return torch.where(r2 < d**2, 0.5 * r2, d * (torch.sqrt(r2) - 0.5 * d))

    @torch.no_grad()
    def solve_differential_extinction(
        self, c_minus_cbar: torch.Tensor, Sigma_inv: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        den = (
            torch.einsum(
                "i,bij,j->b", self.reddening_vec, Sigma_inv, self.reddening_vec
            )
            + self.lambda_E
        )
        return num / (den + 1e-8)
    ...
    def __call__(
        ...
        for group in groups:
            ...
            extinction_correction = E.unsqueeze(1) * self.reddening_vec.unsqueeze(
                0
            )  # [N, B-1]
            ...
            r2 = torch.einsum(
                "ni,nij,nj->n", corrected_residuals, group_covs, corrected_residuals
            )
    ...
    def compute_color_distance(
        ...
        E_opt = self.solve_differential_extinction(
            c_diff.unsqueeze(0), cov_combined.unsqueeze(0)
        )[0]
        ...
        distance = torch.sqrt(
            torch.einsum(
                "i,ij,j", corrected_diff, torch.inverse(cov_combined), corrected_diff
            )
        )
...
class DataAwareColorPrior:
    ...
    def compute_prior_weight(
        self, images: torch.Tensor, metadata: Dict, groups: List[List[int]]
    ) -> torch.Tensor:
        """
    ...
    def __call__(self, *args, **kwargs):
        ...
        if "groups" in kwargs and "images" in kwargs:
            weights = self.compute_prior_weight(
                kwargs["images"], kwargs.get("metadata", {}), kwargs["groups"]
            )
...
class QuasarMorphologyDetector:
    ...
    def is_quasar_like(self, images: torch.Tensor) -> bool:
        ...
        center_flux = images[
            :,
            :,
            images.shape[2] // 2 - 2 : images.shape[2] // 2 + 2,
            images.shape[3] // 2 - 2 : images.shape[3] // 2 + 2,
        ].mean()
