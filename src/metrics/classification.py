...
def compute_classification_metrics(
    y_true: torch.Tensor, y_probs: torch.Tensor, threshold: float = 0.5
) -> Dict[str, float]:
    """
...
def operating_point_selection(
    y_true: torch.Tensor, y_probs: torch.Tensor, method: str = "f1_max"
) -> Dict[str, float]:
    """
...
def _recall_fixed_threshold(
    y_true: np.ndarray, y_probs: np.ndarray, target_recall: float = 0.9
) -> Dict[str, float]:
...
def _precision_fixed_threshold(
    y_true: np.ndarray, y_probs: np.ndarray, target_precision: float = 0.9
) -> Dict[str, float]:
