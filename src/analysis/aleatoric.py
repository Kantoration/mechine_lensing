...
def _logistic_ci(
    logits: torch.Tensor, logit_var: torch.Tensor, z: float = 1.96
...
def selection_scores(
    ...
    *,
    strategy: Literal[
        "entropy", "low_margin", "wide_ci", "high_brier", "nll", "hybrid"
    ] = "entropy",
) -> torch.Tensor:
    """
    ...
    *,
    ...
    if class_balance is None:
        ...
        if len(pos_indices) > 0 and k_pos > 0:
            ...
            _, pos_topk = torch.topk(
                pos_scores, min(k_pos, len(pos_indices)), largest=True
            )
            selected_indices.append(pos_indices[pos_topk])

        if len(neg_indices) > 0 and k_neg > 0:
            neg_scores = scores[neg_indices]
            _, neg_topk = torch.topk(
                neg_scores, min(k_neg, len(neg_indices)), largest=True
            )
...
def ensemble_disagreement(prob_members: List[torch.Tensor]) -> Dict[str, torch.Tensor]:
    ...
    vote_entropy = -(
        mean_prob * _safe_log(mean_prob) + (1 - mean_prob) * _safe_log(1 - mean_prob)
    )
    ...
    for i in range(num_members):
        for j in range(i + 1, num_members):
            ...
            kl_ij = p_i * _safe_log(p_i / p_j) + (1 - p_i) * _safe_log(
                (1 - p_i) / (1 - p_j)
            )

            # KL(p_j || p_i)
            kl_ji = p_j * _safe_log(p_j / p_i) + (1 - p_j) * _safe_log(
                (1 - p_j) / (1 - p_i)
            )
...
def indicators_to_dataframe_dict(
    indicators: AleatoricIndicators, sample_ids: Optional[List[str]] = None
) -> Dict[str, any]:
    """
