...
class UnifiedConfig:
    ...
    def _validate_configuration(self):
        ...
        if (
            self.model.type in ["ensemble", "physics_informed"]
            and not self.model.architectures
        ):
...
def load_config(config_path: Optional[Union[str, Path]] = None) -> UnifiedConfig:
    ...
    if config_path is None:
        ...
        default_paths = ["configs/unified.yaml", "config.yaml", "config.yml"]
...
def create_config_from_args(args) -> UnifiedConfig:
    ...
    system_config = SystemConfig(
        device=getattr(args, "device", "auto"), seed=getattr(args, "seed", 42)
    )
